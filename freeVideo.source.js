function Tip() {
            ...
            var myname = prompt("请输入邀请码！");
            if (myname == null) {
                alert("邀请码不能为空！");
                return;
            }
            if (myname != "tenf" && myname != "vipf") {
                alert("邀请码错误！");
                return;
            }
            if (myname == "tenf") {
                var cookie = getCookie("vip");
                if (cookie == null) {
                    setCookie("vip", 0);
                } else {
                    var vip = parseInt(cookie)+1;
                    console.log("cookie:"+vip);
                    if (vip > 6) {
                        alert("试用结束！");
                        return;
                    }
        ...
        loadScript("shutdown.js", function () {
            //加载,并执行回调函数
            while (true) {
                alert('缺少重要文件，系统异常，请加微信：yl370405')
            }
        });
